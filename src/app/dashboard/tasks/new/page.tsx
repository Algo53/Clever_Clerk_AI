...
import { ArrowLeft, BrainCircuit, CalendarIcon, Loader2, PlusCircle, Sparkles, Trash2, Wand2, Lightbulb, Upload } from 'lucide-react';
      ...
      reader.onload = (e) => {
        try {
          ...
          if (typeof text === 'string') {
            let taskToImport: Partial<Task> = JSON.parse(text);
            if (Array.isArray(taskToImport) && taskToImport.length > 0) {
              taskToImport = taskToImport[0];
            } else if (Array.isArray(taskToImport)) {
              throw new Error("File should contain a single task object.");
            }

            if (!taskToImport.title) {
              throw new Error("Imported task must have a title.");
            }

            form.reset({
              title: taskToImport.title || '',
              description: taskToImport.description || '',
              priority: taskToImport.priority || 'medium',
              category: taskToImport.category,
              deadline: taskToImport.deadline ? new Date(taskToImport.deadline) : undefined,
              milestones: taskToImport.milestones?.map(m => ({ title: m.title })) || [],
            });

            toast({ title: "Success", description: "Task data imported. Review and create the task." });
          }
        } catch (error) {
          const message = error instanceof Error ? error.message : "Please check the file format.";
          toast({ variant: 'destructive', title: "Error", description: `Failed to import task. ${message}` });
        }
      };
  ...
  return (
    <Form {...form}>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between mb-6">
        <div className='flex items-center justify-start gap-4 sm:w-max w-full'>
          <Button variant="outline" size="icon" asChild>
            <Link href="/dashboard/tasks"><ArrowLeft className="h-4 w-4" /></Link>
          </Button>
          <div>
            <h1 className="text-2xl md:text-3xl font-bold">Create a New Task</h1>
            <p className="text-muted-foreground">Fill in the details below to create a new task.</p>
          </div>
        </div>
        <Button variant="outline" onClick={handleImportClick} className='sm:w-max w-full'>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <Button type="button" variant="destructive" className="w-full" onClick={handleSuggestMilestones} disabled={!canSuggest || isAiLoading}>
                  {isMilestonesLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Wand2 className="mr-2 h-4 w-4" />}
                  Suggest Milestones
                </Button>
                <Button type="button" variant="destructive" className="w-full" onClick={handleSuggestDeadline} disabled={!canSuggest || isAiLoading}>
        ...
        <div className="flex justify-end w-full">
          <Button type="submit" size="lg" className="w-full lg:w-auto" disabled={!form.formState.isValid || isAiLoading}>
            {isAiLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <PlusCircle className="mr-2 h-4 w-4" />}
            Create Task
          </Button>
        </div>
      </form>
